...
{
private:
   fs::path m_value; //virtual path in files.db
   mutable fs::path m_real; //real path in file system

public:
   sce_junction(fs::path value);
...
public:
   ...
   bool create_empty_directory(fs::path source_root, fs::path destination_root) const;

   //create empty file in destination root using path from this junction
   //leaves stream opened for other operations like write
   bool create_empty_file(fs::path source_root, fs::path destination_root, std::ofstream& outputStream) const;

   //create empty file in destination root using path from this junction
   bool create_empty_file(fs::path source_root, fs::path destination_root) const;

   //copy file in destination root using path from this junction
   bool copy_existing_file(fs::path source_root, fs::path destination_root) const;

   //copy file with specific size in destination root using path from this junction
   bool copy_existing_file(fs::path source_root, fs::path destination_root, std::uintmax_t size) const;
