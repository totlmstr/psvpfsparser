...
bool FilesDbParser::linkDirpaths(const std::set<fs::path> real_directories)
{
   m_output << "Linking dir paths..." << std::endl;

   std::mutex logMutex;

   std::vector<fs::path> real_directories_vector;
   std::vector<fs::path> real_directories_vector_uppercase;
...
bool FilesDbParser::linkFilepaths(const std::set<fs::path> real_files, std::uint32_t fileSectorSize)
{
   m_output << "Linking file paths..." << std::endl;

   std::mutex logMutex;

   std::vector<fs::path> real_files_vector;
   std::vector<fs::path> real_files_vector_uppercase;
   ...
   std::for_each(
      ...
      [&](sce_ng_pfs_file_t &file) {
         ...
         const auto size = file.path().file_size();
...
int FilesDbParser::matchFileLists(const std::set<fs::path>& files)
{
   ...
   std::vector<fs::path> files_vector_uppercase;
...
int FilesDbParser::parse()
{
   ...
   fs::path filepath = m_titleIdPath / "sce_pfs" / "files.db";
   if(!fs::exists(filepath))
   {
