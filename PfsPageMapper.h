...
public:
   PfsPageMapper(std::shared_ptr<ICryptoOperations> cryptops, std::shared_ptr<IF00DKeyEncryptor> iF00D, std::ostream& output, const unsigned char* klicensee, fs::path titleIdPath);
...
public:
   ...
   int load_page_map(fs::path filepath, std::map<std::uint32_t, std::string>& pageMap) const;
